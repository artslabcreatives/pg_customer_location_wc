<?php
/**
 * PGs by Customer Location from Artslab Creatives for WooCommerce - Main Class
 *
 * @version 1.4.0
 * @since   1.0.0
 *
...
if ( ! class_exists( 'ALC_PGs_by_Customer_Location' ) ) :

final class ALC_PGs_by_Customer_Location {
	...
	/**
	 * @var   ALC_PGs_by_Customer_Location The single instance of the class
	 * @since 1.0.0
	 */
	protected static $_instance = null;

	/**
	 * Main ALC_PGs_by_Customer_Location Instance
	 *
	 * Ensures only one instance of ALC_PGs_by_Customer_Location is loaded or can be loaded.
	 *
	 * @version 1.0.0
	 * @since   1.0.0
	 *
	 * @static
	 * @return  ALC_PGs_by_Customer_Location - Main instance
	 */
	...
	/**
	 * ALC_PGs_by_Customer_Location Constructor.
	 *
	 * @version 1.4.0
	 * @since   1.0.0
	 *
	...
	function admin() {
		...
		// Version update
		if ( get_option( 'alc_pg_by_location_wc_version', '' ) !== $this->version ) {
	...
	/**
	 * Add PGs by Customer Location from Artslab Creatives settings tab to WooCommerce settings.
	 *
	 * @version 1.4.0
	 * @since   1.0.0
	 */
	...
	function version_updated() {
		...
		if ( version_compare( get_option( 'alc_pg_by_location_wc_version', '' ), '1.1.0', '<' ) && function_exists( 'WC' ) && ( $gateways = WC()->payment_gateways->payment_gateways() ) ) {
			foreach ( $gateways as $key => $gateway ) {
				foreach ( array( 'country', 'state', 'postcode' ) as $type ) {
					foreach ( array( 'include', 'exclude' ) as $incl_or_excl ) {
						if ( false !== ( $old_value = get_option( 'alc_pg_by_location_wc_' . $type . '_' . $incl_or_excl . '_' . $key, false ) ) ) {
							delete_option( 'alc_pg_by_location_wc_' . $type . '_' . $incl_or_excl . '_' . $key );
							$new_value = get_option( 'alc_pg_by_location_wc_' . $type . '_' . $incl_or_excl, array() );
							$new_value[ $key ] = $old_value;
							update_option( 'alc_pg_by_location_wc_' . $type . '_' . $incl_or_excl, $new_value );
						}
					}
				}
			}
		...
		// Update version
		update_option( 'alc_pg_by_location_wc_version', $this->version );
	}
